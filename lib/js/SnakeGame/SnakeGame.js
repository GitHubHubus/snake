...
export default class SnakeGame extends BaseSnakeGame {
    ...
    constructor (params) {
        super({
            field: new Field('main', {tile: {width: 10, height: 10, indent: 1}, width: 150, height:150, border: true}),
            onEndGame: params.onEndGame
        });
    ...
    _handleSnakeMoving(event) {
        const exist = this._field.isExistTile(event.p);

        if (exist && !this._field.isLockTile(event.p)) {
            let point = this._snake.decreaseSnake();
            this._field.unlockTile(point);
            this._field.cleanTile(point);
        } else {
            if (!exist || this._snake.isSnake(event.p)) {
                this._handleEndGame();
            } else {
                this._score.set();
                this._addPurpose();
